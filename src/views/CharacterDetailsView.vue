...
<template>
    ...
    <div v-else-if="data" class=" min-h-screen py-8">
        <div class="max-w-4xl mx-auto bg-white dark:bg-gray-700  shadow-md rounded-lg overflow-hidden md:flex">
            ...
            <div class="p-8 flex-grow">
                ...
                <h2 class="mt-1 text-2xl leading-tight font-semibold text-gray-800 dark:text-white">{{ data.name }}</h2>

                <div class="mt-2 flex items-center text-gray-600 dark:text-white">
                    <Icon icon="material-symbols:info-outline" class="mr-2" />
                    <span class="font-semibold">ID:</span> {{ data.id }}
                </div>
                <div class="mt-1 flex items-center text-gray-600 dark:text-white">
                    <Icon :icon="genderIcon" class="mr-2" />
                    <span class="font-semibold">Gender:</span> {{ data.gender }}
                </div>

                <div class="mt-4">
                    <h3 class="flex items-center text-lg font-semibold text-gray-700 dark:text-white">
                        <Icon icon="material-symbols:location-on-outline" class="mr-2" /> Location:
                    </h3>
                    <p class="mt-1 text-gray-600 dark:text-white">{{ data.location.name }}</p>
                </div>
                ...
                <div class="mt-2">
                    ...
                    <p class="mt-1 text-gray-600 dark:text-white">{{ data.origin.name }}</p>
                </div>
                <UIButton class="mt-4" @click="goBack">Ir atras</UIButton>
            </div>
        </div>
    </div>
</template>